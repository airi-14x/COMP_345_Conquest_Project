...
ViewState::ViewState(vector<string> o, bool oc, map<string, int> p, int n)
{
    owners = o;
    ownershipChange = oc;
    players = p;
...
void Map::exchangeCountry(Country* country, string playerName) {
    country->setPlayerName(playerName);
    Notify();
}

ViewState* Map::getState() {
    // counter used to count how many country each player owners
    // map<player's name, counter for number of countries>
    ...
    {
        bool ownAll = true;
        string owner = conti[i]->getCountry(0)->getPlayerName();
        string oldOwner = conti[i]->owner;
        ...
        for (int j = 0; j < conti[i]->getCntsSize(); j++)
        {
            ...
            // get the name of the player who owns the country
            string name = conti[i]->getCountry(j)->getPlayerName();

            // if the player does not exist in the map
            // insert them into the map and initialize their country counter to 1
            // otherwise increment the player's country count
            map<string, int>::iterator it = players.find(name);
            if ( it == players.end() ) {
              players.insert(std::pair<string,int>(name, 1));
            } else {
              it->second = it->second + 1;
            }

            if(ownAll && owner.compare(name) == 0){
                conti[i]->owner = owner;
            } else {
                conti[i]->owner = "nobody";
                ownAll = false;
            }
        }

        if(ownAll && oldOwner.compare(conti[i]->owner) != 0){
            ownershipChange = true;
        }

        owners.push_back(conti[i]->getName() + " is owned by " + conti[i]->owner);
    }

    return new ViewState(owners, ownershipChange, players, getMapSize());
}
...
void Country::setHuman(bool h)
{
    human = h;
}

bool Country::isHuman() {
    return human;
}
