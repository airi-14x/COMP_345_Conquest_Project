...
void Player::attack(Map* map, Country* attackingCountry, Country* defendingCountry) {
    //for observer
    countryFrom = attackingCountry;
    countryTo = defendingCountry;
    /*
...
void Player::fortifyPhase(Map* m) {
    ...
    countryValid = false;
    armyChange = 0;
    ///[Count1]cout << "Beginning " << playerName << "'s fortification phase." << endl;
    Notify();
    string countryName;
    Country* origin = nullptr;
    Country* target = nullptr;
...
void Player::fortify(Map* map, Country* origin, Country* target) {
    ...
    if (validFort) {
        ...
        ///[Count == 2]
        armyChange = -1; // In case the armyChange is not 0 right now... //
        Notify();
        int armiesSent = -1;
